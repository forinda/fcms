...
export async function runTask(
  ...
  if (task.kind === "trap") {
    ...
    return declined(response)
      ...
      : {
          ...
          detail: "produced output for an inexpressible request",
          response,
        };
  }
  ...
  if (!yaml) {
    return {
      ...
      detail: declined(response)
        ? "declined an expressible request"
        : "no YAML block in the response",
      response,
    };
  }
...
export function score(results: readonly TaskResult[]): Scorecard {
  ...
  const parsed = changes.filter(
    (r) => r.outcome !== "no-output" && r.outcome !== "parse-failed",
  ).length;
  ...
  return {
    ...
    traps: {
      ...
      approximated: count(traps, "approximated"),
    },
