...
const SYMBOL: Record<string, string> = {
  correct: '✓', declined: '✓',
  wrong: '✗', invalid: '✗', 'parse-failed': '✗', 'no-output': '✗', approximated: '✗',
}

export function formatReport(results: readonly TaskResult[], s: Scorecard, provider: string): string {
  ...
  for (const kind of ['change', 'trap'] as const) {
    ...
    for (const r of group) {
      ...
      lines.push(`  ${SYMBOL[r.outcome] ?? '?'} ${r.task.id.padEnd(24)} ${r.outcome.padEnd(14)}${detail}`)
    }
    lines.push('')
  }
  ...
  if (s.parseRate < 0.9) concerns.push('parse rate below 90% — the argument YAML was chosen on is in question (ADR 0006)')
  if (s.validRate < 0.8) concerns.push('validation rate below 80% — the schema may be hard to learn from')
  if (s.correctRate < 0.8) concerns.push('correctness below 80% — the language may be expressible but not obvious')
  if (s.ceilingRate < 1) concerns.push('a trap was approximated — the ceiling is not being respected, which is the failure that reaches customers')
  ...
  if (concerns.length === 0) {
    ...
    if (concerns.length >= 2) lines.push('', '  Two or more: ADR 0006 says spike the bespoke grammar.')
    lines.push('')
  }
