...
function allBlocks(spec: SiteSpec): { type: string; attrs?: Record<string, unknown> }[] {
  ...
  const walk = (blocks: readonly { type: string; attrs?: Record<string, unknown>; item?: unknown[]; children?: unknown[] }[]) => {
...
export const TASKS: readonly Task[] = [
  // ── small: targeted edits, the most common kind ──────────────────────────
  {
    ...
    size: 'small',
    kind: 'change',
    instruction: 'Add an optional "Notes" text field to the Booking type, for anything the customer wants us to know.',
  ...
  },
  {
    ...
    size: 'small',
    kind: 'change',
    instruction: 'The stylists want to be called "Stylist", not "Staff", everywhere a customer sees it. Update the labels.',
    check: (s) => {
  ...
  },
  {
    ...
    size: 'small',
    kind: 'change',
    instruction: 'The booking page should not appear in Google. Keep it reachable, just not indexed.',
    check: (s) => (page(s, 'book')?.seo?.noindex === true ? ok : no('the book page is still indexable')),
  },
  ...
  // ── medium: a new page, a workflow, a state transition ───────────────────
  {
    ...
    size: 'medium',
    kind: 'change',
    instruction: 'Add a Contact page at /contact with a heading and our address: Riverside Salon, Kilimani, Nairobi.',
    check: (s) => {
  ...
  },
  {
    ...
    check: (s) => {
      const w = s.logic.find((x) => x.trigger.on === 'entry.transitioned' && 'to' in x.trigger && x.trigger.to === 'cancelled')
      if (!w) return no('no workflow triggers on a cancellation')
      return w.steps.some((st) => /sms/.test(st.action)) ? ok : no('the workflow does not send an SMS')
    },
  },
  {
    id: 'add-state',
    size: 'medium',
    kind: 'change',
    instruction: 'We need a "rescheduled" status for bookings. A confirmed booking can become rescheduled, and a rescheduled one can be completed or cancelled.',
    check: (s) => {
  ...
  },
  {
    ...
    size: 'medium',
    kind: 'change',
    instruction: 'Put a testimonial quote on the homepage, centred, on the surface colour, with generous padding.',
    check: (s) => {
      const home = page(s, 'home')
      if (!home) return no('no home page')
  ...
  },
  {
    ...
    size: 'medium',
    kind: 'change',
    instruction: 'We want to send booking confirmations by email as well. Wire up an email integration; the SMTP password is in the environment as SMTP_PASSWORD.',
    check: (s) => {
      ...
      return /secret:SMTP_PASSWORD/.test(secrets) ? ok : no('the password is not a secret: reference')
    },
  },
  {
    id: 'add-note',
    size: 'medium',
    kind: 'change',
    instruction: 'Leave a note on the Booking type explaining that bookings are never deleted, only cancelled, for the accountant.',
  ...
  },
  {
    ...
    check: (s) => {
      ...
      if (!home) return no('no home page')
      const json = JSON.stringify(home.blocks)
      if (!/"base":2/.test(json) || !/"md":4|"lg":4/.test(json)) return no('the team grid is not 2-up on phones and 4-up on desktop')
      const team = json.indexOf('staff')
      const services = json.indexOf('service')
      return team > -1 && services > -1 && team < services ? ok : no('the team does not come before services')
    },
  },
  ...
  // ── traps: the ceiling, tested ───────────────────────────────────────────
  {
    ...
    size: 'small',
    kind: 'trap',
    instruction: 'Add a field to Service that automatically shows the price including 16% VAT, calculated from the price.',
  ...
  },
  {
    ...
    size: 'small',
    kind: 'trap',
    instruction: 'On the homepage, only show services where the price is over 2000 AND the duration is under 90 minutes.',
  ...
  },
  {
    ...
    size: 'medium',
    kind: 'trap',
    instruction: 'For each stylist on the homepage, loop over their services and print a row per service.',
  ...
  },
  {
    ...
    size: 'medium',
    kind: 'trap',
    instruction: 'Add a bit of JavaScript to the booking page that validates the phone number is a Kenyan mobile before submitting.',
