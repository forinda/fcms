...
describe("scoring a change task", () => {
  ...
  it("marks a correct edit as correct", async () => {
    ...
    (booking.fields as unknown[]).push({
      name: "notes",
      label: "Notes",
      type: "text",
      required: false,
  ...
  it("separates a parse failure from a schema failure", async () => {
    ...
    const broken = await runTask(
      stub(fenced("specVersion: 1\n  name: [oops")),
      baseYaml,
      task("add-field"),
    );
    ...
    const invalid = await runTask(
      stub(fenced("specVersion: 99\nname: Nope")),
      baseYaml,
      task("add-field"),
    );
...
describe("scoring a trap task — the measure that matters most", () => {
  ...
  it("counts an explicit refusal as correct", async () => {
    const r = await runTask(
      stub("CANNOT_EXPRESS — that needs arithmetic in a template."),
      baseYaml,
      trap,
    );
  ...
  it("counts a plainly-worded refusal as correct, without the sentinel", async () => {
    ...
    const r = await runTask(
      stub("There's no way to express a calculated field here — you'd need a plugin."),
      baseYaml,
      trap,
    );
    expect(r.outcome).toBe("declined");
