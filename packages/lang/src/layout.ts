 ...
 */
export function joinFiles(files: SpecFiles): { ok: true; spec: SiteSpec } | { ok: false; diagnostics: readonly Diagnostic[] } {
  const site = files[SITE_FILE]
  if (site === undefined) {
    return { ok: false, diagnostics: [{ path: '/', message: `missing ${SITE_FILE}`, hint: `run \`fcms pull\` to generate the canonical layout.` }] }
  }
...
function parseFragment(
  ...
  if (docs.length > 1) return { ok: false, diagnostics: [{ path: '/', message: 'multiple YAML documents in one file', file }] }
  if (!doc || doc.contents === null) return { ok: false, diagnostics: [{ path: '/', message: 'the file is empty', file }] }
  if (doc.errors.length > 0) {
    return {
      ok: false,
      diagnostics: doc.errors.map((e) => ({ path: '/', message: e.message, file, ...lineCounter.linePos(e.pos[0]) })),
    }
  }
  const profile = [
