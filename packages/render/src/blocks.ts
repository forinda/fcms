...
function safeHref(value: unknown): string | undefined {
  ...
  if (/^(https?:)?\/\//i.test(href) || href.startsWith('/') || href.startsWith('#') || href.startsWith('mailto:') || href.startsWith('tel:')) {
    return href
  }
  return undefined
}
...
export const CORE_BLOCKS: Record<string, BlockType> = Object.fromEntries(
  [
    // ── Layout ──────────────────────────────────────────────────────────────
    define({
      name: 'section', summary: 'A full-width band of the page.', attrs: [], layout: true,
      render: ({ className, children }) => el('section', { class: `fx-section ${className}` }, children),
    }),
    define({
      name: 'stack', summary: 'Children in a vertical column.', attrs: [], layout: true,
      render: ({ className, children }) => el('div', { class: `fx-stack ${className}` }, children),
    }),
    define({
      name: 'row', summary: 'Children in a horizontal row that wraps.', attrs: [], layout: true,
      render: ({ className, children }) => el('div', { class: `fx-row ${className}` }, children),
    }),
    define({
      name: 'grid', summary: 'Children in a responsive grid.', attrs: [], layout: true,
      render: ({ className, children }) => el('div', { class: `fx-grid ${className}` }, children),
    }),

    // ── Content ─────────────────────────────────────────────────────────────
    define({
      name: 'heading', summary: 'A headline.', attrs: ['text', 'level'],
    ...
    define({
      name: 'text', summary: 'A paragraph of plain text.', attrs: ['text'],
      render: ({ className, attrs }) => el('p', { class: className }, str(attrs['text'])),
    }),
    define({
      name: 'richtext', summary: 'Formatted text authored by the site owner.', attrs: ['html'],
    ...
    define({
      name: 'image', summary: 'A picture.', attrs: ['src', 'alt', 'width', 'height'],
      render: ({ className, attrs }) =>
        el('img', {
          class: className,
          src: str(attrs['src']),
    ...
    define({
      name: 'button', summary: 'A link styled as a button.', attrs: ['label', 'to'],
    ...
    define({
      name: 'card', summary: 'A titled block of content, usually one row of a list.',
      attrs: ['heading', 'body', 'image', 'to'], variants: ['plain', 'elevated'],
      render: ({ className, attrs, children }) => {
        const href = safeHref(attrs['to'])
        const inner = fragment(
          str(attrs['image']) ? el('img', { src: str(attrs['image']), alt: '', loading: 'lazy' }) : null,
          str(attrs['heading']) ? el('h3', {}, str(attrs['heading'])) : null,
          str(attrs['body']) ? el('p', {}, str(attrs['body'])) : null,
          children,
        )
        return el('article', { class: `fx-card ${className}` }, href ? el('a', { href }, inner) : inner)
      },
    }),
    define({
      name: 'list', summary: 'Renders one child template per row of a query.', attrs: [], layout: true,
      render: ({ className, children }) => el('div', { class: `fx-grid ${className}` }, children),
    }),
    define({
      name: 'divider', summary: 'A horizontal rule.', attrs: [],
      render: ({ className }) => el('hr', { class: className }),
    }),
    define({
      name: 'spacer', summary: 'Vertical space.', attrs: [],
      render: ({ className }) => el('div', { class: className, 'aria-hidden': 'true' }),
    }),

    // ── Structure ───────────────────────────────────────────────────────────
    define({
      name: 'nav', summary: 'Site navigation.', attrs: ['links'],
      render: ({ className, attrs }) => {
        const links = Array.isArray(attrs['links']) ? attrs['links'] : []
        return el('nav', { class: `fx-nav ${className}`, 'aria-label': 'Main' },
          el('ul', {}, ...links.map((raw_) => {
            const link = raw_ as Record<string, unknown>
            const href = safeHref(link['to'])
            return el('li', {}, href ? el('a', { href }, str(link['label'])) : str(link['label']))
          })))
      },
    }),
    define({
      name: 'footer', summary: 'Page footer.', attrs: ['text'], layout: true,
      render: ({ className, attrs, children }) =>
        el('footer', { class: className }, str(attrs['text']) ? el('p', {}, str(attrs['text'])) : null, children),
    }),
