  ...
  pages: [
    {
      ...
      title: 'Home',
      blocks: [
        { type: 'heading', attrs: { text: 'Book with us', level: 1 }, style: { padding: 'lg', textColor: 'token:color.brand' } },
        {
          type: 'list',
          style: { cols: { base: 1, md: 3 }, gap: 'md' },
          data: { from: 'service', where: [{ field: 'active', op: 'eq', value: true }], sort: { field: 'name', dir: 'asc' }, limit: 12 },
          item: [{ type: 'card', attrs: { heading: '{{ item.name }}', body: '{{ item.price | currency }}' } }],
        },
      ],
    },
    { key: 'service-detail', path: '/services', title: 'Service', collection: 'service', blocks: [{ type: 'heading', attrs: { text: '{{ entry.name }}' } }] },
  ],
})
...
describe('output is escaped by default (doc 05)', () => {
  it('escapes entry content coming from a query', () => {
    const hostile = staticSource({ service: [{ id: 'x', slug: 'x', name: '<script>alert(1)</script>', price: 1, active: true }] })
  ...
  it('drops event-handler attributes and javascript: URLs', () => {
    const page = { ...home, blocks: [{ type: 'button', attrs: { label: 'x', to: 'javascript:alert(1)', onclick: 'alert(1)' } }] } as typeof home
  ...
  it('escapes `</script>` inside JSON-LD', () => {
    const nasty = staticSource({ service: [{ id: '1', slug: 'a', name: '</script><img src=x onerror=alert(1)>', active: true }] })
...
describe('queries (ADR 0009 §1)', () => {
  it('filters, sorts and limits', () => {
    const rows = runQuery(source, { from: 'service', where: [{ field: 'active', op: 'eq', value: true }], sort: { field: 'name', dir: 'asc' }, limit: 12 })
  ...
  it('is total when a field does not exist rather than throwing', () => {
    expect(() => runQuery(source, { from: 'service', where: [{ field: 'nope', op: 'eq', value: 1 }], limit: 5 })).not.toThrow()
  })
})
...
describe('templates resolve in scope (ADR 0001)', () => {
  ...
  it('formats currency in the configured locale, not a hardcoded one', () => {
    ...
    const british = renderPage(home, { spec, source, locale: { locale: 'en-GB', currency: 'GBP' } }).html
...
describe('`when` gates a subtree', () => {
  it('omits a block whose condition fails', () => {
    const page = { ...home, blocks: [{ type: 'heading', attrs: { text: 'Hidden' }, when: { field: 'site.name', op: 'eq', value: 'Other' } }] } as typeof home
    expect(renderPage(page, { spec, source }).html).not.toContain('Hidden')
  })

  it('keeps a block whose condition holds', () => {
    const page = { ...home, blocks: [{ type: 'heading', attrs: { text: 'Shown' }, when: { field: 'site.name', op: 'eq', value: 'Test Salon' } }] } as typeof home
