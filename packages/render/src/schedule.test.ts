...
describe("existing bookings remove slots", () => {
  ...
  it("applies the buffer on both sides", () => {
    ...
    const outside = slots([booking(at(20), 30)], tuesday9to12, {
      slot: { minutes: 30, buffer: 15 },
    });
    ...
    const clashing = slots([booking(at(9, 30), 30)], tuesday9to12, {
      slot: { minutes: 30, buffer: 15 },
    });
...
describe("failing safe", () => {
  ...
  it("skips a resource with no slug or id", () => {
    const source = staticSource({
      staff: [{ name: "Nameless", workingHours: tuesday9to12 }],
      booking: [],
    });
    expect(generateSchedule(source, derived, { now: TUESDAY })).toEqual([]);
  });
...
describe("lead time", () => {
  it("hides slots inside the lead time", () => {
    ...
    const withLead = generateSchedule(
      source,
      { ...derived, window: { days: 1, leadTime: { hours: 2 } } },
      { now: midMorning },
    );
    // Only slots from 11:00 survive a two-hour lead time.
    expect(
      withLead.every((s) => Date.parse(String(s["startsAt"])) >= midMorning.getTime() + 7_200_000),
    ).toBe(true);
