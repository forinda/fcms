...
const RADIUS_SCALE: Record<string, string> = {
  none: '0', xs: '2px', sm: '4px', md: '8px', lg: '16px', xl: '24px', '2xl': '9999px',
}

const WIDTHS: Record<string, string> = { full: '100%', container: '72rem', narrow: '42rem' }
const BORDERS: Record<string, string> = { none: 'none', hairline: '1px solid var(--color-border, #e5e5e5)', strong: '2px solid var(--color-border, #d4d4d4)' }
const SHADOWS: Record<string, string> = { none: 'none', sm: '0 1px 3px rgb(0 0 0 / 0.08)', md: '0 4px 16px rgb(0 0 0 / 0.12)' }
...
export function themeCss(theme: Theme): string {
  ...
  for (const [name, value] of Object.entries(theme.colors)) lines.push(`  --color-${name}: ${value};`)
  for (const [name, value] of Object.entries(theme.typeScale)) lines.push(`  --type-${name}: ${value};`)
  for (const [name, value] of Object.entries(theme.radius ?? {})) lines.push(`  --radius-${name}: ${value};`)
...
function invariant(style: StyleProps): Declarations {
  ...
  if (typeof s['background'] === 'string') {
    d['background'] = s['background'] === 'none' ? 'transparent'
      : s['background'].startsWith('token:') ? tokenVar(s['background'])
  ...
  if (typeof s['align'] === 'string') d['align-items'] = s['align'] === 'start' || s['align'] === 'end' ? `flex-${s['align']}` : String(s['align'])
  if (typeof s['justify'] === 'string') {
    d['justify-content'] = s['justify'] === 'between' ? 'space-between'
      : s['justify'] === 'start' || s['justify'] === 'end' ? `flex-${s['justify']}` : String(s['justify'])
  }
  return d
}
 ...
 */
export function blockCss(className: string, style: StyleProps | undefined, custom?: string): string {
  ...
  if (style) {
    ...
    for (const bp of ['sm', 'md', 'lg', 'xl'] as const) {
      ...
      if (Object.keys(d).length) {
        rules.push(`@media (min-width:${BREAKPOINT_PX[bp]}px){.${className}{${declarationsToCss(d)}}}`)
      }
    }
