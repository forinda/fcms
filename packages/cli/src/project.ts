...
export function loadProject(
  root: string,
): { ok: true; project: Project } | { ok: false; diagnostics: readonly Diagnostic[] } {
  ...
  try {
    ...
    return {
      ...
      diagnostics: [
        {
          path: "/",
          message: `no site.yaml in ${root}`,
          hint: "a spec directory needs site.yaml at its root.",
        },
      ],
    };
  }
  ...
  return {
    ok: true,
    project: { root, spec: joined.spec, source: staticSource(data), files: read },
  };
}
