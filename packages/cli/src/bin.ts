...
function comingInPhase0b(name: string, what: string): Command {
  return new Command(name)
    ...
    .allowUnknownOption()
    .action(() => {
      console.error(`${yellow('not yet')} \`fcms ${name}\` needs a server to talk to — it arrives with Phase 0b.`)
...
export function buildProgram(): Command {
  ...
  program
    ...
    .action((dir: string, options: { check?: boolean }) => process.exit(fmt(resolve(dir), options.check === true)))
